...
public class Rental {
     ...
     * @throws IllegalArgumentException if the user is too young to see the movie
     */
    public Rental(IUser aUser, Movie aMovie, int nofRentalDays, LocalDate rentaldate) {
        if (isValidUser(aUser) && isValidMovie(aMovie) && isValidRentalDays(nofRentalDays)) {
            this.user = aUser;
            aUser.getRentals().add(this);
            this.movie = aMovie;
     ...
     */
    public int calcRemainingDaysOfRental(LocalDate date) {
        if (date == null) {
            throw new IllegalArgumentException("missing date!");
        }
        Period remainingDays = Period.between(rentalDate, date);
        remainingDays.plusDays(rentalDays);

        return remainingDays.getDays(); //diff.intValue();
    }
